...
extern char* StrSHA256(const char* str, long long length, char* sha256) {
	...
	*/
	long length_pp;	// the length of the string after padding
	...
	for (i = 0; i < length; i++) str_pp[i + 3 - 2 * (i % 4)] = str[i];	// copy str
	...
	*/
	...
	/*
		Step 3. update the hash value $H^{(t)}$
	*/
	char *str_pp_end;
	long W[64];
	...
	for (str_pp_end = str_pp + length_pp; str_pp < str_pp_end; str_pp += 64) {
		...
		// compute the intermediate hash value $H^{(t)}$ 
